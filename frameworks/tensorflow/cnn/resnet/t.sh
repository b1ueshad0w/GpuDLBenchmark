...
CUDA_VISIBLE_DEVICES=$deviceId python ${script_path} --train_dir=${train_dir} --batch_size=$batch_size --epochs=$epochs --device_id=$deviceId --xla=True --use_datasets=True
end=`date +%s.%N`
